...
define_opcodes!(
    ...
    // TODO: Figure out what the 7 values are here
    END_ASYNC_FOR = 54 (--),
    ...
    JUMP_IF_FALSE_OR_POP = 111 (condition -- condition[if jump {1} else {0}]),
    JUMP_IF_TRUE_OR_POP = 112 (condition -- condition[if jump {1} else {0}]),
    JUMP_ABSOLUTE = 113 (--),
    ...
    MAKE_FUNCTION = 132 (defaults[if oparg & 0x01 {1} else {0}],
                        kwdefaults[if oparg & 0x02 {1} else {0}],
                        annotations[if oparg & 0x04 {1} else {0}],
                        closure[if oparg & 0x01 {1} else {0}],
    ...
    CALL_FUNCTION_EX = 142 (callable, args, kwargs[if oparg & 0x01 {1} else {0}] -- res),
    SETUP_WITH = 143 (mgr -- exit, exc[if jump {3 + 3} else {0}], enter[if jump {0} else {1}]),
    EXTENDED_ARG = 144 (--),
    ...
    SETUP_ASYNC_WITH = 154 (res -- res[if jump {0} else {1}], excs[if jump {6} else {0}]),
